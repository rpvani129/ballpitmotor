...
import { normalizeExtractedServiceDocument } from "@/lib/service-record-extraction";
...
export default async function ServiceImportReviewPage({ params, searchParams }: { params: Promise<{ id: string; importId: string }>; searchParams: Promise<Record<string,string>> }) {
  ...
  return <main className="dashboard-main"><Link className="back-link" href={`/dashboard/vehicles/${id}`}>← Back to vehicle</Link><section className="page-title compact-title"><p className="eyebrow">{vehicle.business_id} · IMPORT REVIEW</p><h1>Check before saving</h1><p>{imported.file_name} · Nothing has been added to {vehicle.name} yet.</p></section>
    {query.duplicate && <p className="alert">This exact document was already uploaded. The original import is shown below; no duplicate was created.</p>}
    {query.error && <p className="alert">The import could not be saved. Check the required fields and try again.</p>}
    {imported.status === "failed" ? <div className="empty-state"><strong>The document could not be read.</strong><p>{imported.extraction_error || "Try a clearer scan or enter the record manually."}</p><Link className="button primary" href={`/dashboard/vehicles/${id}/service/upload`}>Try another file</Link></div> : imported.status === "committed" ? <div className="success-message">This import has already been added to service history.</div> : imported.status === "review" && imported.extracted_data ? <ImportReviewForm vehicleId={id} importId={importId} initial={normalizeExtractedServiceDocument(imported.extracted_data)} /> : <div className="empty-state"><strong>Still processing.</strong><p>Refresh this page in a moment.</p></div>}
  </main>;
}
